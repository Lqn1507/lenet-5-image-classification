...
x_train, x_test, y_train, y_test = train_test_split(x, y, random_state=42, train_size = .99)
...
for x_single in x_test:
    y_single = model(torch.unsqueeze(x_single, 0))
    y_result.append(y_single)
y_result = torch.squeeze(torch.stack(y_result))
